...
echo "[4/7] Non-stream paper analyze (mock)..."
...
echo "[5/7] Stream paper analyze sequential (mock)..."
...
echo "[6/7] Stream paper analyze parallel (mock)..."
...
echo "[7/7] Batch paper analyze (mock)..."
batch_json="$(mktemp)"
curl -fsS -X POST "$BASE_URL/v1/papers/analyze/batch" \
  -F 'options_json={"mock_mode":true,"stream_mode":"parallel","parallel_limit":2,"angles":["主题","方法"]};type=application/json' \
  -F "files=@${pdf_file};type=application/pdf" \
  -F "files=@${pdf_file};type=application/pdf" >"$batch_json"
python3 -c 'import json,sys; obj=json.load(open(sys.argv[1])); assert obj.get("total")==2 and obj.get("succeeded")==2' "$batch_json"
